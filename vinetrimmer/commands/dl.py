...
from vinetrimmer.config.api import PlayReadyAPI
...
@click.option("-d", "--delay", type=int, default=None,
...
def result(ctx, service, quality, range_, wanted, alang, slang, audio_only, subs_only, chapters_only, audio_description,
           list_, keys, cache, no_cache, no_subs, no_audio, no_video, no_chapters, atmos, vbitrate: int, abitrate: int, channels, no_mux, mux, delay, selected, *_, **__):
    def ccextractor():
    ...
    for title in titles.with_wanted(wanted):
        if not first and delay:
            d = delay + random.randint(int(-delay/5.0), (delay/5.0))
        ...
        try:
           ...
           # title.tracks.select_audios(by_language=alang, with_descriptive=audio_description)
            title.tracks.select_subtitles(by_language=slang, with_forced=slang)
        except ValueError as e:
            log.error(f" - {e}")
            continue
        ...
        for track in title.tracks:
            ...
            if track.encrypted:
                ...
                if not track.key:
                    ...
                    content_keys = []
                    if title.source in ["AMZN", "DSNP", "ATVP", "STRP", "MAX", "PCOK", "HULU"]:
                        playready_api = PlayReadyAPI(
                            title=title,
                            track=track,
                            service=service
                        )
                        content_keys = playready_api.get_keys()
